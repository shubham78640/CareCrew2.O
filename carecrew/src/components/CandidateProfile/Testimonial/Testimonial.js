...
function Testimonial() {
  ...
  return (
    <>
    <Box sx={{display:"flex", flexDirection:"Column", gap:"50px" ,padding:{sm:"10%", xs:"7%"}, marginTop:{sm:"25%", xs:"1200px", justifyContent:"center"}}}>
   <Box sx={{fontSize:"30px", fontWeight:"400" ,textAlign:"center"}}>Testimonial </Box>
    <Slider {...settings}>
              ...
              <NavLink style={{textDecoration:"none"}} to={item.link}  onClick={() => {
                    window.scrollTo({
                      ...
                      behavior: "smooth",
                                  ...
                                  <p>{item.paragraph}</p>
                      <Stack spacing={1}>
                      <Rating  name="read-only" defaultValue={item.id} precision={0.5} readOnly />
                      </Stack>
              </NavLink>
            </div>
          </div>
