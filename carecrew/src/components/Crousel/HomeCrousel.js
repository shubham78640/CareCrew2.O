    ...
    <Carousel variant="dark">
      {crouselData.map((items) => (
        <Carousel.Item>
          ...
          />
          <Carousel.Caption>
            <motion.h5 initial={{ x: "-100%" }} whileInView={{ x: 0 }} style={{ color: "white" }}>{items.title}</motion.h5>
            <motion.p initial={{ x: "-100%" }} whileInView={{ x: 0 }} style={{ color: "white" }}>{items.description}</motion.p>
          </Carousel.Caption>
        </Carousel.Item>
      ))}
    </Carousel>
